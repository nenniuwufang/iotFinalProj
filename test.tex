...
In the door lock module, we used GD32 chip with W5500 Ethernet chip as a network relay to perform power-off and power-on operations by accepting AT commands from the back-end.

To the backend development, we used Java and Kotlin language, the main stream Spring Boot framework and MySQL database as the basis for the development and running on the cloud platform.
...
The simulation software, we choose the Proteus, which is a professional circuit simulation software.
The software is used to simulate the hardware circuit, and it can simulate the hardware circuit and the software program.

We use the Rigol DS1054Z, which is a 4-channel oscilloscope.
...
To the DC source, we use the DS2482, which is a 16-bit ADC.
The DC source is used to test the voltage of the hardware circuit, and it can display the voltage of the hardware circuit.

The last validation is the multimeter, we use the Rigol DM3058, which is a 5.5-digit multimeter.
...
For the software validation environment, we used JDK11, MySQL8 to program.

We also write a few user cases to test the system's robustness. By using Socket protocol to create the TCP connection, and sending AT command to GD32 hardware.

For the back-end functional validation, we mainly use Postman to send post requests. And the back-end will return the validation result.
...
\begin{table}[H]
    ...
    \resizebox{\linewidth}{!}{
        \begin{tabular}{|c|c|c|c|c|}
            ...
            \hline
            open\_01 & Door lock opened & User swipes the card normally for verification & Door opened successfully & Passed \\
            \hline
            open\_02 & Card swipe fails & Unknown user swipes the card for verification  & Card is not bound & Passed      \\
            \hline
            open\_03 & \begin{tabular}[c]{@{}c@{}}The user does not\\have the permission\end{tabular} & \begin{tabular}[c]{@{}c@{}}The user who has been frozen by the\\administrator to swipe the card for verification\end{tabular} & \begin{tabular}[c]{@{}c@{}}The door opening permission\\is frozen\end{tabular} & Passed      \\
            \hline
        \end{tabular}
    }
    Sheet 1: User Cases
