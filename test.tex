...
For the Smart E-ink NFC card, we integrated the NFC card simulater and a E-ink screen. The information on the E-ink screen can be updated by chip ST25DV, which can communicate with the phone by NFC protocol.

The smart E-ink NFC card has its own unique ID in the form of a \texttt{UUID}. And the Smart Door System can use the \texttt{UUID} to identify the card. The \texttt{UUID} is stored in the ST25DV chip, and the ST25DV chip is connected to the STM32L051 chip through the I2C bus. The STM32L051 chip is responsible for the display of the E-ink screen and the control of the NFC communication.

The \texttt{UUID} of the smart E-ink NFC card is not unique, and it is a unique ID that you can find in the NFC card and the smart E-ink screen. In order to make the \texttt{UUID} unique, the smart E-ink NFC card must be attached to an NFC tag. The \texttt{UUID} is not simply a unique ID, but the unique ID that can be found in the NFC card, as well as the smart E-ink screen.

It is important to note that the \texttt{UUID} is not necessarily unique. If a tag has multiple \texttt{UUID}s, the card will not be able to identify the card, and the Smart Door System will not be able to detect the smart E-ink screen.

\subsubsection{Smart Door Lock System}

For the Smart Door Lock System, we integrated the NFC card reader, the magnetic door lock and a 
